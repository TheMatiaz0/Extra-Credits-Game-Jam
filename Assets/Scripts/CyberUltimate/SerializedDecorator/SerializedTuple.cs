...
{
    [Serializable]
    public struct SerializedTuple<T1, T2> : IStructuralComparable, IStructuralEquatable, IComparable, IComparable<(T1, T2)>, IEquatable<(T1, T2)>, ITuple
    {
        [SerializeField]
        public T1 X;
        [SerializeField]
        ...
        {
            return ((X, Y) as IEqualityComparer).Equals(other, comparer);
        }

        int IStructuralEquatable.GetHashCode(IEqualityComparer comparer)
        {
            return ((X, Y) as IEqualityComparer).GetHashCode(comparer);
        }
    }
}
